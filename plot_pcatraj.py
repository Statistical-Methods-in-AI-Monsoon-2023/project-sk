...
parser.add_argument('--weight_path',
                    type=str,
                    help="path to the weight folder")
...
parser.add_argument('--include_bn',
                    type=bool,
                    help='To include the batch norm in the parameters',
                    default=True)
...
def vis(rank, model, dirn1, dirn2, criterion, steps, indices, output, args):
    ...
    for s, step in enumerate(steps):
        # idx is [a, b]
        a, b = step
        for i, d1, d2, k in zip(model.parameters(), dirn1.parameters(),
                                dirn2.parameters(), vis_model.parameters()):
...
def get_eigvecs(args):
    models = []
    fsorted = sorted(os.listdir(args.weight_path),
                     key=lambda x: int(os.path.splitext(x)[0]))
    concats = []
    global finalmodel
    for file in fsorted:
        if file.endswith(".pt"):
            model = torch.load(os.path.join(args.weight_path, file),
                               map_location=device)
            model.eval()
            finalmodel = model
            ...
            if not args.include_bn:
                for p in model.parameters():
                    fn = torch.zeros_like if len(
                        p.shape) == 1 else torch.ones_like
...
def plot_trajectory(x_vals, y_vals):
    ...
    if args.show:
        ...
        plt.savefig(f"results/{args.model}-pca.png", transparent=True)
...
if __name__ == "__main__":
    ...
    for param, m_param in zip(dirn1.parameters(), model.parameters()):
        ...
        param.data = v_ten1[total_len:total_len + param_len].reshape(
            param.shape)  #* torch.linalg.norm(m_param)
        total_len += param_len
    ...
    for param, m_param in zip(dirn2.parameters(), model.parameters()):
        param_len = param.reshape(-1).shape[0]
        param.data = v_ten2[total_len:total_len + param_len].reshape(
            param.shape)  #* torch.linalg.norm(m_param)
        total_len += param_len
    ...
    mesh = torch.cat(
        [mesh_x.unsqueeze(0), mesh_y.unsqueeze(0)],
        0).permute(1, 2, 0).reshape(-1, 2)

    num_per_proc = mesh.shape[0] // nprocs
    steps = [
        mesh[i * num_per_proc:(i + 1) * num_per_proc, :]
        for i in range(nprocs - 1)
    ]
    steps.append(mesh[(nprocs - 1) * (num_per_proc):, :])
    indices = [
        torch.arange(i * num_per_proc, (i + 1) * num_per_proc)
        for i in range(nprocs - 1)
    ]
    ...
    for i in range(nprocs):
        p = mp.Process(target=vis,
                       args=[
                           i, model, dirn1, dirn2, criterion, steps[i],
                           indices[i], output, args
                       ])
        p.start()
        workers.append(p)
