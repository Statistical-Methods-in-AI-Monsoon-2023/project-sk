...
parser.add_argument('--model_name', type=str, default='resm1', help='Name of the model')
...
def plot_loss_acc_move2D(model, dirn1, dirn2, criterion, dataloader, device):
    ...
    np.save('2d'+args.model_name+'.npy', np.array([[loss], [accs], [x], [y]]))
...
model1 = give_model('weights/'+args.weight_path, device)
...
dirn1 = VGG(vgg_cfgs[vgg_id], True)
dirn1.to(device)
...
dirn2 = VGG(vgg_cfgs[vgg_id], True)
dirn2.to(device)
